...
class ARController {
    ...
    init() {
        // СНАЧАЛА показываем AR контейнер, потом настраиваем
        this.showARContainer();
    ...
    showARContainer() {
        // Показываем AR контейнер и скрываем стартовую панель
        const arContainer = document.getElementById('ar-container');
        const statusPanel = document.getElementById('status-panel');
        
        if (arContainer) {
            arContainer.classList.remove('hidden');
            console.log('✅ AR контейнер показан');
        }
        
        if (statusPanel) {
            statusPanel.classList.add('hidden');
            console.log('✅ Стартовая панель скрыта');
        }
        ...
        setTimeout(() => {
            ...
            if (scene) {
                ...
                console.log('🔄 Сцена перерисована');
            }
        }, 200);
    }
    waitForAFrame() {
    ...
    setup() {
        ...
        if (!this.model) {
           ...
           // Попробуем найти модель другими способами
            this.model = document.querySelector('[gltf-model]');
            if (!this.model) {
                console.error('❌ Модель не найдена вообще');
                this.showModelError();
            return;
        }
    }
        
console.log('✅ Модель найдена:', this.model.id || this.model.tagName);
...
setTimeout(() => {
    ...
    setTimeout(() => testElement.remove(), 3000);
        ...
        // Проверяем события загрузки модели
...
setupModelEvents() {
        ...
        // События загрузки GLTF модели
        this.model.addEventListener('model-loaded', () => {
            console.log('✅ Модель загружена успешно');
            document.getElementById('model-status').textContent = "Модель: Загружена ✓";
            
            // Сохраняем начальный масштаб из атрибутов модели
            ...
            if (scale) {
                ...
                console.log('📏 Начальный масштаб:', this.initialScale);
            }
        });
        
        this.model.addEventListener('model-error', (e) => {
            console.error('❌ Ошибка загрузки модели:', e.detail);
            document.getElementById('model-status').textContent = "Модель: Ошибка загрузки";
            this.showModelError();
        });
    }
    ...
    setupControls() {
        ...
        document.removeEventListener('click', this.handleControlClick);
        // Добавляем новые
