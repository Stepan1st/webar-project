...
class WebARApp {
    ...
    loadModel() {
        ...
        this.model.addEventListener('model-error', (e) => this.onModelError(e));
         ...
         // Создаем ассет для модели
        const asset = document.createElement('a-asset-item');
        asset.setAttribute('id', 'robot-model');
        asset.setAttribute('src', 'assets/models/robot.glb'); 
        
        // Добавляем ассет в сцену
        let assets = this.scene.querySelector('a-assets');
        if (!assets) {
...
window.addEventListener('load', () => {
    ...
    if (!gl) {
        ...
        errorMessage.textContent = 'Ваш браузер не поддерживает WebGL. Обновите браузер или используйте Chrome/Safari.';
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        const errorPanel = document.getElementById('error-panel');
        const errorMessage = document.getElementById('error-message');
        errorMessage.textContent = 'Ваше устройство не поддерживает доступ к камере. Убедитесь, что вы используете HTTPS.';
        errorPanel.classList.remove('hidden');
        return;
    }
